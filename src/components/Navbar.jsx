        ...
        <div className="container flex flex-wrap items-center justify-between p-4">
          ...
          <div className={`flex md:order-2 space-x-3 md:space-x-0 ${ hidden ? 'hidden' : '' }`}>
            ...
            <div onClick={() => setOpenFloat(float => !float)}>
              <img ref={refs.setReference} src={user.photo ? `https://prw8fl-5000.csb.app/api/images/${user.photo}` : user.gender === 'female' ? female : male} alt="" className="w-10 h-10 rounded-full object-cover border border-gray-100" />
