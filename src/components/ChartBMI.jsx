...
function ChartBMI({bmi}) {
  ...
  return (
    <div className="relative">
      ...
      <p className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 font-medium text-lg text-${bmi < 18.5 ? 'yellow' : bmi < 25 ? 'blue' : bmi < 30 ? 'orange' : 'red'}-500`}>{parseFloat(bmi)}</p>
    </div>
  )
}
