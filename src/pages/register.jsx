...
function Register() {
  ...
  const submit = async(e) => {
    ...
    try {
      ...
      let response = await fetch('https://prw8fl-5000.csb.app/api/auth/register', {
        method: 'POST',
      ...
      if (response.status === 400) {
        dispacth(setToast({type: 1, message: 'Email sudah terdaftar'}))
        return
      }
      dispacth(setToast({type: 0, message: 'Berhasil mendaftar'}))
        ...
        <form className="text-center shadow border border-gray-200 rounded-2xl w-full max-w-sm p-8 mx-auto" onSubmit={(e) => submit(e)}>
          ...
          <div className="flex flex-col text-start mb-2">
            ...
            <svg xmlns="http://www.w3.org/2000/svg" id="Outline" viewBox="0 0 24 24" className="w-5 h-5 absolute -translate-y-1/2 top-1/2 ml-3"><path d="M12,12A6,6,0,1,0,6,6,6.006,6.006,0,0,0,12,12ZM12,2A4,4,0,1,1,8,6,4,4,0,0,1,12,2Z"/><path d="M12,14a9.01,9.01,0,0,0-9,9,1,1,0,0,0,2,0,7,7,0,0,1,14,0,1,1,0,0,0,2,0A9.01,9.01,0,0,0,12,14Z"/></svg>
              <input type="text" name="name" id="name-input" className="p-2.5 w-full bg-emerald-50 rounded-lg text-sm border-2 border-emerald-300 hover:bg-white hover:border-emerald-400 focus:outline-none focus:bg-white pl-9" disabled={loading} required/>
            </div>
          </div>
          <div className="flex flex-col text-start mb-2">
          ...
          <button type="submit" className="btn btn-primary w-full mt-16 mb-2" disabled={loading}>{loading ? 'Memuat...' : 'Daftar'}</button>
